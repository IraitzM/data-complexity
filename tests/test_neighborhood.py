...
class TestNeighborhood2(unittest.TestCase):
    ...
    def test_cancer(self):
        ...
        numpy.testing.assert_allclose(model.c_N1(), 0.086116, atol=1e-06)
        numpy.testing.assert_allclose(model.c_N3(), 0.091388, atol=1e-06)
        numpy.testing.assert_allclose(model.c_N4(), 0.534271, atol=1e-01)
        numpy.testing.assert_allclose(model.c_N5(), 0.003515, atol=1e-06)
        numpy.testing.assert_allclose(model.c_N6(), 0.912293, atol=1e-06)

    def test_cancer_100(self):
        model = NeighborhoodMeasures()
        model.fit(self.X[:100], self.y[:100])

        numpy.testing.assert_allclose(model.c_N1(), 0.1, atol=0)
        numpy.testing.assert_allclose(model.c_N3(), 0.08, atol=1e-06)
        numpy.testing.assert_allclose(model.c_N4(), 0.5, atol=1e-01)
        numpy.testing.assert_allclose(model.c_N5(), 0.02, atol=1e-06)
        numpy.testing.assert_allclose(model.c_N6(), 0.7953, atol=1e-06)
